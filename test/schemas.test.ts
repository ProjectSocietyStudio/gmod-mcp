...
describe("schemas", () => {
  ...
  it("Session applies default arrays", () => {
...
describe("isCallAllowed (gate de confirmation)", () => {
  ...
  it("lets unguarded tools through", () => {
    expect(isCallAllowed(plain, {}, [])).toBe(true);
  });

  it("refuses a guarded tool without confirm", () => {
    expect(isCallAllowed(guarded, {}, [])).toBe(false);
  });

  it("lets a guarded tool through with confirm:true", () => {
    expect(isCallAllowed(guarded, { confirm: true }, [])).toBe(true);
  });

  it("lets a guarded tool through when it is in the allowlist", () => {
